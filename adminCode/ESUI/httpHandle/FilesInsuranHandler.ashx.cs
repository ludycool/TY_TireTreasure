...
{
    /// <summary>
    /// FilesInsuranHandler 的摘要说明
    /// </summary>
    public class FilesInsuranHandler : IHttpHandler
    {

        public TT_FilesInsuranBiz OPBiz = new TT_FilesInsuranBiz();
        ...
        public HttpReSultMode addImg(HttpContext context)
        {
            ...
            if (context.Request.Files.Count > 0)
            {
                for (int i = 0; i < context.Request.Files.Count; i++)
                {
                    ...
                    {
                        TT_FilesInsuran file = AddImgOne(out jmsg, hpFile, lenth, SourceTable, ToId, ShowName);
                        if (file != null)
                        {
        ...
        /// <returns></returns>
        private TT_FilesInsuran AddImgOne(out  HttpReSultMode rsm, HttpPostedFile hpFile, int lenth, string SourceTable, string ToId, string ShowName)
        {
            ...
            try
            {
                ...
                string HttpFileRoute = ConfigurationManager.AppSettings["HttpFileRoute"].ToString();//文件保存本地路径根目录
                string FileRelativeRoute = ConfigurationManager.AppSettings["InsuranRoute"].ToString();//文件保存相对目录
                //给文件取随及名 
